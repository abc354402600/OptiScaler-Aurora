...
Update: active private Streamline binding was subsequently implemented in [the binding follow-up](STREAMLINE_BINDING_20260916.md), including separate runtime/FG readiness and the second-device path. The source audit below describes the problem that follow-up addresses; provider-global ordering is still outstanding.
